...
OUTPUT_FILE = "model_output_balanced_rf"                                        
METADATA = './all_data_final_key.csv'
...
COLUMNS = ['fire_occurrence','acres_burned','avg_dp_temp','avg_rel_hum','avg_wb_temp','avg_wind_speed','precip','pop_density','latitude','longitude']
ALL_FEATURES = ['avg_dp_temp','avg_rel_hum','avg_wb_temp','avg_wind_speed','precip','pop_density','latitude','longitude']
#constants for model                                                                                                                                                             
...
def generate_model (models, depth, cutoff_val, features, sample_size, balanced, output_file):
    ...
    data = pd.read_csv(METADATA, sep=',', dtype={'county':str,'fire_occurrence':int, 'acres_burned': int, 'avg_dp_temp': float, 'avg_rel_hum': float, 'avg_wb_temp': float, 'avg_wind_speed': float, 'precip': float, 'pop_density': float, 'latitude':float, 'longitude':float}, header=0)
